...
[1] http://coneural.org/florian/papers/05_cart_pole.pdf
...
class CartPole(object):
    ...
    def step(self, force):
        ...
        # Compute new accelerations as given in "Correct equations for the dynamics of the cart-pole system by Razvan V. Florian (http://florian.io). # [1]
        st = sin(self.theta)
        ct = cos(self.theta)
    ...
    def get_scaled_state(self):
        ...
        """
        return [0.5 * (self.x + self.position_limit) / self.position_limit, (self.dx + 0.75) / 1.5,
                0.5 * (self.theta + self.angle_limit_radians) / self.angle_limit_radians, (self.dtheta + 1.0) / 2.0]
