...
class Population(object):
    ...
    def initialize(self, fitness_function, config):
        self.config = config
        self.population_size = config.pytorch.population_size
        self.fitness_function = fitness_function
        self.fitness_criterion = stat_functions.get(config.pytorch.fitness_criterion)
        if self.fitness_criterion is None and not config.pytorch.no_fitness_termination:
            raise RuntimeError('Unexpected fitness criterion [{}]'.format(config.pytorch.fitness_criterion))
    ...
    def fit(self, n=None):
        if self.config.pytorch.no_fitness_termination and (n is None):
        ...
        while n is None or k < n:
            ...
            if not self.config.pytorch.no_fitness_termination:
                fv = self.fitness_criterion(members_fitness)
                if fv >= self.config.pytorch.fitness_threshold:
                    break
